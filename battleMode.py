    ...
    def update(self,timePassed):
        ...
        for i in xrange(len(enemySprites)-1,-1,-1):
            ...
            if(isCollision(AposX,AposY,AsizeX,AsizeY,BposX,BposY,BsizeX,BsizeY)):
                del enemySprites[i]
                ExplosionSprites.append(EnemyExplosion(Vector2(BposX,BposY)))
                bombExplosionSprites.append(BombExplosion(Vector2(BposX-128,BposY-128),"images/bombexplosion.png"))
                score[0].score1+=10
                explode.play()
                self.dflag = True
        CposX = player2.pos.x
    ...
    def update(self,timePassed):
        ...
        for i in xrange(len(enemySprites)-1,-1,-1):
            ...
            if(isCollision(AposX,AposY,AsizeX,AsizeY,BposX,BposY,BsizeX,BsizeY)):
                del enemySprites[i]
                ExplosionSprites.append(EnemyExplosion(Vector2(BposX,BposY)))
                bombExplosionSprites.append(BombExplosion(Vector2(BposX-128,BposY-128),"images/bombexplosion_2.png"))
                score[0].score2+=10
                explode.play()
                self.dflag = True
        CposX = player1.pos.x
...
class BombExplosion(pygame.sprite.Sprite):
    def __init__(self,pos,image_pass):
        pygame.sprite.Sprite.__init__(self)
        self.image = load_image(image_pass, -1)
        self.pos = pos
        self.counter = 0
        self.maxcount = 50
        self.flag = True
